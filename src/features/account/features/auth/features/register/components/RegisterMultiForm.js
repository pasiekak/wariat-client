...
import { NestedFirstForm } from "./FirstForm";
import { NestedSecondForm } from "./SecondForm";
import "../styles/registerMultiForm.css";
...
const RegisterMultiForm = () => {
  ...
  const { t: tErr } = useTranslation("schemas", {
    keyPrefix: "schemas.register",
  });
  const { t: tApi } = useTranslation("apiMessages", {
    keyPrefix: "apiMessages.register",
  });
  ...
  return (
    <FormProvider {...methods}>
      ...
      >
        ...
        {step === 2 ? (
          <input type="submit" className="submitButton" value={t("button")} />
        ) : null}
