...
const EmailCodeForm = ({ regData }) => {
  ...
  const { t: tErr } = useTranslation("schemas", {
    keyPrefix: "schemas.emailCode",
  });
  const { t: tApi } = useTranslation("apiMessages", {
    keyPrefix: "apiMessages.emailCode",
  });
  ...
  const onSubmit = async (data) => {
    setShowSpinner(true);
    let response = await accountActions.verifyEmailCodeAndRegister(
      regData,
      data.code,
    );
    ...
    if (response?.success === false) {
      ...
      setApiMessage(response.message);
      let apiResponse = await accountActions.login(
        regData.username,
        regData.password,
      );
    ...
    <form className="EmailCodeForm" onSubmit={handleSubmit(onSubmit)}>
      ...
      {showSpinner ? (
        ...
        <>
          ...
          <span color={success ? "green" : "red"}>
            {apiMessage ? tApi(apiMessage) : null}
          </span>
