    ...
    <div className="Filters">
      <div className="filters-wrapper">
        ...
        <div className="marks">
          ...
          {marks && (
            <MultiSelectFilter
              className={`MultiSelectFilter${isMarkExpanded ? " open" : ""}`}
              key={1}
              data={marks}
              set={updateSelectedMarks}
              t={t}
            />
          )}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            onClick={() => setIsMarkExpanded(!isMarkExpanded)}
            width="20"
            height="20"
            fill="currentColor"
            className={isMarkExpanded ? "open" : ""}
            viewBox="0 0 16 16"
          >
        ...
        <div className="categories">
          ...
          {categories && (
            <MultiSelectFilter
              className={`MultiSelectFilter${isCatExpanded ? " open" : ""}`}
              key={2}
              data={categories}
              set={updateSelectedCategories}
              t={t}
            />
          )}
          <svg
        ...
        <div className="buttons">
          <Button variant="dark" onClick={() => refresh()}>
            {t("confirm-button")}
          </Button>
        </div>
      </div>
