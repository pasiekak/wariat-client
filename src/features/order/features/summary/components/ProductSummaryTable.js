...
import "../styles/product-summary-table.css";

const ProductSummaryTable = ({ products, withDiscountPrices }) => {
  const { t } = useTranslation(null, {
    keyPrefix: "components.order.summary.products-summary-table",
  });
  return (
    <table id="products-summary-table">
      ...
      <tbody>
        {products.map((product) => {
          const total = Number(product.priceBrutto * product.quantity).toFixed(
            2,
          );
          return (
            <tr key={product.id}>
