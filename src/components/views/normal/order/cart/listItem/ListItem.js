...
const ListItem = ({item}) => {
    ...
    const handleInputNumberChange = (e) => {
        ...
        if(value > item.maxQuantity) {
            ...
            if(value < 1) { setValue('quantity', 1) } else {
    ...
    return (
        <ListGroup.Item key={item.id}>
            <div className="item-left">
                <img src={item.mainImageId > 0 ? `/api/images/${item.mainImageId}` : defaultImage} alt=""/>
            ...
            <div className="item-right">
                <Form onChange={handleSubmit(onChange)}>
                    ...
                    <Form.Select {...register('quantity')}>
                        {[...Array((item.maxQuantity >= 10) ? 10 : item.maxQuantity).keys()].map((value) => (
                            value < 9 ? 
                            <option key={value + 1} value={value + 1}>
                            {value + 1}
                            </option> : <option key={value + 1} value={value+1}>9+</option>
                        ))}
                    </Form.Select>}
                </Form>
